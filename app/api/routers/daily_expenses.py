...
async def get_daily_expense_by_id(doc_id: int): # Renamed function
    """
    ...
    expense = database.get_daily_expense_by_id(doc_id) # Updated table
    if not expense:
...
async def create_daily_expense(daily_expense: DailyExpense): # Renamed function and parameter
    """
    ...
    try:
        new_expense = database.add_daily_expense(daily_expense) # CORRECTED FUNCTION CALL
...
async def update_daily_expense_api(doc_id: int, updates: Dict[str, Any]):
    """
    ...
    success = database.update_daily_expense(doc_id, updates)
    if not success:
    ...
    updated_expense = database.get_daily_expense_by_id(doc_id)
